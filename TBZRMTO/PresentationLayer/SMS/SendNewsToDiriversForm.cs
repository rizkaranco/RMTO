...
{
    public partial class SendNewsToDiriversForm : Form
    {
        public SendNewsToDiriversForm()
        {
            InitializeComponent();
        }
        ...
        private void SendNewsToDiriversForm_Load(object sender, EventArgs e)
        {
        ...
        {
            HPS.BLL.TrafficBLL.BLLTraffic_TFactory DriverFactory = new HPS.BLL.TrafficBLL.BLLTraffic_TFactory();
            DataTable DriverDataTable = new DataTable();
            string Conditioin = string.Empty;
            DriverFactory.SelectDriverMobile_nvc(Conditioin, ref DriverDataTable);
            ...
            NewsFactory.GetAllByCondition(string.Format("[News_T].[StartDate_nvc] <= '{0}' And [News_T].[EndDate_nvc] >= '{0}'",NewsFactory.ServerJalaliDate.ToString()), ref NewsDataTable);
            NewsDataTable.Columns.Add(new DataColumn("Checked_bit", Type.GetType("System.Boolean")));
            NewsDataTable.Columns["Checked_bit"].AllowDBNull = true;
            NewsDataTable.Columns["Checked_bit"].DefaultValue = false;
            this.NewsdataGridView .AutoGenerateColumns = false;
            this.NewsdataGridView.DataSource = NewsDataTable;
        }
        ...
        private void Sendbutton_Click(object sender, EventArgs e)
        {
            ...
            {
                DataRow[] DriverRow = SelectedDriver.Select("Checked_bit = 'True'");
                if (DriverRow != null && DriverRow.Length > 0)
                {
                    ...
                    if (frm.True_bit)
                    {
                        ...
                        StringBuilder MessegeText = new StringBuilder();
                        string MobileList;
                        MobileList = string.Empty;

                        foreach (DataRow rw in DriverRow)
                        {
                        ...
                        DataTable SelectedNews = new DataTable();
                        SelectedNews = (DataTable)this.NewsdataGridView.DataSource;
                        DataRow[] NewsRow = SelectedNews.Select("Checked_bit = 'True'");
                        foreach (DataRow rw in NewsRow)
                        {
                            MessegeText.AppendLine(String.Format("{0}:{1}", rw["Title_nvc"].ToString(), rw["News_nvc"].ToString()));
                        }
        ...
        private void NewsReversbutton_Click(object sender, EventArgs e)
        {
            DataTable Newsdt = (DataTable)this.NewsdataGridView.DataSource;
            if (this.NewsdataGridView.RowCount > 0)
            {
        ...
        private void NewsSearchtextBox_TextChanged(object sender, EventArgs e)
        {
            DataTable Dt = (DataTable)this.NewsdataGridView.DataSource;
            Dt.DefaultView.RowFilter = String.Format("Title_nvc Like '%{0}%'", NewsSearchtextBox .Text); 
        }
