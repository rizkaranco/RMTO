...
{
    public partial class AllInServiceTypeReportForm : Hepsa.Core.PL.BaseForm
    {
        public AllInServiceTypeReportForm()
        {
            InitializeComponent();
        }
        ...
        private void ShowButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //HPS.Reports.ReportDoc.AllInServiceTypeReport rpt = new HPS.Reports.ReportDoc.AllInServiceTypeReport(FromDatefaDatePicker.Text,  FromTimeTextBox.Text, ToDatefaDatePicker.Text,  ToTimeTextBox.Text);
                ...
                AllInServiceTypestiReport.Dictionary.Synchronize();
                AllInServiceTypestiReport.Dictionary.Databases.Clear();
                AllInServiceTypestiReport.Dictionary.Databases.Add(new Stimulsoft.Report.Dictionary.StiSqlDatabase("Connection", Hepsa.Core.Common.ConnectionString.ConnectionString));
                AllInServiceTypestiReport.Compile();
                HPS.BLL.PortPlacesBLL.BLLPortPlaces_TFactory PortPlaceFactory = new HPS.BLL.PortPlacesBLL.BLLPortPlaces_TFactory();
                AllInServiceTypestiReport["organ"] = new HPS.BLL.SettingsBLL.BLLSetting_TFactory().GetBy(new BLL.SettingsBLL.BLLSetting_TKeys() { SettingID_int = 1029 }).Value_nvc.ToString();
                AllInServiceTypestiReport["@FromDate_nvc"] = FromDatefaDatePicker.Text;
                AllInServiceTypestiReport["@FromTime_nvc"] = FromTimeTextBox.Text;
                AllInServiceTypestiReport["@ToDate_nvc"] = ToDatefaDatePicker.Text;
                AllInServiceTypestiReport["@ToTime_nvc"] = ToTimeTextBox.Text;
                AllInServiceTypestiReport["fromdate_vc"] = FromDatefaDatePicker.Text;
                AllInServiceTypestiReport["todate_vc"] = ToDatefaDatePicker.Text;
                AllInServiceTypestiReport["date_vc"] = PortPlaceFactory.ServerJalaliDate + "   " + PortPlaceFactory.ServerTime; ;
                AllInServiceTypestiReport["UserName_vc"] = HPS.Common.CurrentUser.user.UserName_nvc;
                AllInServiceTypestiReport["FromTime_vc"] = FromTimeTextBox.Text;
                AllInServiceTypestiReport["ToTime_vc"] = ToTimeTextBox.Text;
                AllInServiceTypestiReport.Render();
                this.stiViewerControl1.Report = AllInServiceTypestiReport;
            }
            catch (Exception ex)
            {
        ...
        private void AllInServiceTypeReportForm_Load(object sender, EventArgs e)
        {
            HPS.BLL.BillBLL.BLLBill_TFactory Factory = new HPS.BLL.BillBLL.BLLBill_TFactory();
