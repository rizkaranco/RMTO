...
{
    public partial class CarLadBillAndCancelAndTurnReportForm : Form
    {
        public CarLadBillAndCancelAndTurnReportForm()
        {
            InitializeComponent();
        }
        ...
        private void ShowButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (DriverWithDuplicateCarCheckBox.Checked)
                {
                    DriverWithDuplicateCarReport.Dictionary.Synchronize();
                    DriverWithDuplicateCarReport.Dictionary.Databases.Clear();
                    DriverWithDuplicateCarReport.Dictionary.Databases.Add(new Stimulsoft.Report.Dictionary.StiSqlDatabase("Connection", Hepsa.Core.Common.ConnectionString.ConnectionString));
                    DriverWithDuplicateCarReport.Compile();
                    DriverWithDuplicateCarReport["organ"] = new HPS.BLL.SettingsBLL.BLLSetting_TFactory().GetBy(new BLL.SettingsBLL.BLLSetting_TKeys() { SettingID_int = 1029 }).Value_nvc.ToString();
                    DriverWithDuplicateCarReport["date_vc"] = TrafficFactory.ServerJalaliDate + " " + TrafficFactory.ServerTime;
                    DriverWithDuplicateCarReport["UserName_vc"] = HPS.Common.CurrentUser.user.UserName_nvc;
                    DriverWithDuplicateCarReport["fromdate_vc"] = FromDatefaDatePicker.Text;
                    DriverWithDuplicateCarReport["todate_vc"] = ToDatefaDatePicker.Text;
                    DriverWithDuplicateCarReport["fromTime_vc"] = FromTimeTextBox.Text;
                    DriverWithDuplicateCarReport["toTime_vc"] = ToTimeTextBox.Text;

                    DriverWithDuplicateCarReport["@FromDate_nvc"] = FromDatefaDatePicker.Text;
                    DriverWithDuplicateCarReport["@ToDate_nvc"] = ToDatefaDatePicker.Text;
                    DriverWithDuplicateCarReport["@FromTime_nvc"] = FromTimeTextBox.Text;
                    DriverWithDuplicateCarReport["@ToTime_nvc"] = ToTimeTextBox.Text;

                    if (string.IsNullOrEmpty(LaderTypeList_nvc))
                        DriverWithDuplicateCarReport["@LaderTypeID_nvc"] = DBNull.Value;
                    else
                        DriverWithDuplicateCarReport["@LaderTypeID_nvc"] = Convert.ToString(LaderTypeList_nvc);

                    if (string.IsNullOrEmpty(CarSerialnumbertextBox.Text))
                        DriverWithDuplicateCarReport["@SerialPlate_nvc"] = DBNull.Value;
                    else
                        DriverWithDuplicateCarReport["@SerialPlate_nvc"] = CarSerialnumbertextBox.Text;

                    if (string.IsNullOrEmpty(NumberPlate_nvctextBox.Text))
                        DriverWithDuplicateCarReport["@NumberPlate_nvc"] = DBNull.Value;
                    else
                        DriverWithDuplicateCarReport["@NumberPlate_nvc"] = NumberPlate_nvctextBox.Text;
                    //@CarCardNumber_nvc
                    if (string.IsNullOrEmpty(CarCardNumber_nvctextBox.Text))
                        DriverWithDuplicateCarReport["@CarCardNumber_nvc"] = DBNull.Value;
                    else
                        DriverWithDuplicateCarReport["@CarCardNumber_nvc"] = CarCardNumber_nvctextBox.Text;
                    if (string.IsNullOrEmpty(DriverCardNumber_nvcTextBox.Text))
                        DriverWithDuplicateCarReport["@DriverCardNumber_nvc"] = DBNull.Value;
                    else
                        DriverWithDuplicateCarReport["@DriverCardNumber_nvc"] = DriverCardNumber_nvcTextBox.Text;
                    DriverWithDuplicateCarReport["@DuplicateDriverCard_vc"] = "ok";

                    DriverWithDuplicateCarReport.Render();
                    stiViewerControl1.Report = DriverWithDuplicateCarReport;
                }
                else
                {
                    ...
                    if (string.IsNullOrEmpty(DriverCardNumber_nvcTextBox.Text))
                        CarLadeBillstiReport["@DriverCardNumber_nvc"] = DBNull.Value;
                    else
                        CarLadeBillstiReport["@DriverCardNumber_nvc"] = DriverCardNumber_nvcTextBox.Text;

                    CarLadeBillstiReport["@DuplicateDriverCard_vc"] = "no";
        ...
        private void CarCardNumber_nvctextBox_TextChanged(object sender, EventArgs e)
        {
