...
{
    public partial class UsersPriceReportForm :Hepsa.Core.PL.BaseForm
    {
        public UsersPriceReportForm()
        {
            InitializeComponent();
        }
        ...
        private void ShowButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                UsersPriceReportstiReport.Dictionary.Synchronize();
                UsersPriceReportstiReport.Dictionary.Databases.Clear();
                UsersPriceReportstiReport.Dictionary.Databases.Add(new Stimulsoft.Report.Dictionary.StiSqlDatabase("Connection", Hepsa.Core.Common.ConnectionString.ConnectionString));
                UsersPriceReportstiReport.Compile();
                UsersPriceReportstiReport["organ"] = new HPS.BLL.SettingsBLL.BLLSetting_TFactory().GetBy(new BLL.SettingsBLL.BLLSetting_TKeys() { SettingID_int = 1029 }).Value_nvc.ToString();
                HPS.BLL.PortPlacesBLL.BLLPortPlaces_TFactory PortPlaceFactory = new HPS.BLL.PortPlacesBLL.BLLPortPlaces_TFactory();

                UsersPriceReportstiReport["@FromDate_nvc"] = FromDatefaDatePicker.Text;
                UsersPriceReportstiReport["@FromTime_nvc"] = FromTimeTextBox.Text;
                UsersPriceReportstiReport["@ToDate_nvc"] = ToDatefaDatePicker.Text;
                UsersPriceReportstiReport["@ToTime_nvc"] = ToTimeTextBox.Text;
                UsersPriceReportstiReport["fromdate_vc"] = FromDatefaDatePicker.Text;
                UsersPriceReportstiReport["todate_vc"] = ToDatefaDatePicker.Text;
                UsersPriceReportstiReport["date_vc"] = PortPlaceFactory.ServerJalaliDate + "   " + PortPlaceFactory.ServerTime; ;
                UsersPriceReportstiReport["UserName_vc"] = HPS.Common.CurrentUser.user.UserName_nvc;
                UsersPriceReportstiReport["FromTime_vc"] = FromTimeTextBox.Text;
                UsersPriceReportstiReport["ToTime_vc"] = ToTimeTextBox.Text;
                UsersPriceReportstiReport.Render();
                this.stiViewerControl1.Report = UsersPriceReportstiReport;

                //HPS.Reports.ReportDoc.UsersPriceReport rpt = new HPS.Reports.ReportDoc.UsersPriceReport(FromDatefaDatePicker.Text, FromTimeTextBox.Text, ToDatefaDatePicker.Text, ToTimeTextBox.Text);
        ...
        private void UsersPriceReportForm_Load(object sender, EventArgs e)
        {
            HPS.BLL.TrafficBLL.BLLTraffic_TFactory Factory = new HPS.BLL.TrafficBLL.BLLTraffic_TFactory();
